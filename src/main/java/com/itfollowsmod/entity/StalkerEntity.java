...
public class StalkerEntity extends PathfinderMob {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        ...
        // ItFollowsMod.LOGGER.info("MAX_HEALTH: {}", ModConfig.STALKER_MAX_HEALTH.get());
        // ItFollowsMod.LOGGER.info("MOVEMENT_SPEED: {}", ModConfig.STALKER_MOVEMENT_SPEED.get());
        // ItFollowsMod.LOGGER.info("ATTACK_DAMAGE: {}", ModConfig.STALKER_ATTACK_DAMAGE.get());
    ...
    public void onAddedToWorld() {
        if (!this.level.isClientSide) {
            ...
            // Debug log
            // ItFollowsMod.LOGGER.info("StalkerEntity Speed Set: {}", this.getAttributeValue(Attributes.MOVEMENT_SPEED));
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(6, new MeleeAttackGoal(this, this.getAttributeValue(Attributes.MOVEMENT_SPEED), true));
    ...
    public void aiStep() {
        ...
        if (ModConfig.STALKER_PREVENT_SLEEP.get()) this.wakeSleepingPlayers();
    ...
    private void removeDuplicateEntities() {
        ...
        //     ItFollowsMod.LOGGER.info("[Stalker] Found stalker with ID: {}", stalker.getId());
        // }
        
        // ItFollowsMod.LOGGER.info("[Stalker] Found {} stalkers in the world", stalkers.size());
        ...
        if (stalkers.size() > 1) {
            ...
            // ItFollowsMod.LOGGER.info("[Stalker] Oldest stalker has ID: {}", oldest.getId());
            
            // If this isn't the oldest entity, remove it
            if (this.getId() != oldest.getId()) {
                // ItFollowsMod.LOGGER.info("[Stalker] Removing duplicate entity with ID: {}", this.getId());
                this.discard();
            } else {
                // If we're the oldest, remove all others
                for (int i = 1; i < stalkers.size(); i++) {
                    StalkerEntity duplicate = stalkers.get(i);
                    // ItFollowsMod.LOGGER.info("[Stalker] Removing duplicate entity with ID: {}", duplicate.getId());
                    duplicate.discard();
                }
            }
        }
        ...
        public void start() {
            if (targetDoorPos != null) {
                ...
                if (state.getBlock() instanceof net.minecraft.world.level.block.DoorBlock) {
                    stalker.level.setBlock(targetDoorPos, state.setValue(net.minecraft.world.level.block.DoorBlock.OPEN, true), 10);
                    stalker.playSound(SoundEvents.FENCE_GATE_OPEN, 1.0F, 1.0F);
                }
            }
        }
    ...
    private void playSound() {
        if (!ModConfig.ENABLE_PROXIMITY_SOUNDS.get()) return;

        long currentTime = this.level.getGameTime();
        if (currentTime - lastSoundPlayTime < SOUND_COOLDOWN + random.nextInt(200)) return; // Randomized delay
    ...
    /**
     * Checks the distance to the player and respawns the entity if too far away, called specifically from wakeSleepingPlayers()
     *
    ...
    private void attemptTeleportNearPlayer(ServerLevel serverWorld) {
        ...
        if (nearestPlayer == null) {
            // ItFollowsMod.LOGGER.info("[Stalker] No nearby player found for teleportation.");
            return;
        }
    
        // boolean shouldTeleport = (this.distanceToSqr(nearestPlayer) > ModConfig.MINIMUM_SPAWN_DISTANCE.get()) && (random.nextDouble() < 0.20); // 20% chance
        boolean shouldTeleport = this.distanceToSqr(nearestPlayer) > ModConfig.MINIMUM_SPAWN_DISTANCE.get();  
    
        /*
        ItFollowsMod.LOGGER.info("[Stalker] Teleport check: Distance={}, MinDistance={}, ChanceRoll={}",
        ...
        */
        ...
        if (spawnPos != null) {
            ...
            /*
            ItFollowsMod.LOGGER.info("[Stalker] Teleported from ({}, {}, {}) to ({}, {}, {})",
                oldX, oldY, oldZ,
    ...
    private void respawnNearby() {
        ...
        if (nearestPlayer == null) {
            // ItFollowsMod.LOGGER.warn("[Stalker] respawnNearby: No player found nearby. Leaving entity at current location.");
            return;
        }

        // Check for the rare chance to spawn right next to player
        boolean spawnNearPlayer = (this.distanceToSqr(nearestPlayer) > ModConfig.MINIMUM_SPAWN_DISTANCE.get()) && (random.nextDouble() < 0.005);
        ...
        if (spawnNearPlayer) {
            ...
            if (spawnPos != null) {
                this.teleportTo(spawnPos.getX() + 0.5, spawnPos.getY(), spawnPos.getZ() + 0.5);
                // ItFollowsMod.LOGGER.info("[Stalker] Respawned directly beside player at {}", spawnPos);
                return;
            }
        }
        ...
        if (spawnPos != null) {
            ...
            if (this.level.getBlockState(this.blockPosition()).getMaterial().isSolid()) {
                ...
                // ItFollowsMod.LOGGER.info("[Stalker] Adjusted position upward to {}", elevatedPos);
            }
        } else {
            // ItFollowsMod.LOGGER.warn("[Stalker] Failed to find spawn position - falling back to vanilla spawn placement");
    ...
    private void vanillaSpawnNearPlayer(ServerLevel serverWorld, Player player) {
        ...
        for (int attempt = 0; attempt < maxTries; attempt++) {
            ...
            if (spawnPos != null) {
                this.teleportTo(spawnPos.getX() + 0.5, spawnPos.getY(), spawnPos.getZ() + 0.5);
                // ItFollowsMod.LOGGER.info("[Stalker] Vanilla spawn succeeded at {}", spawnPos);
                return;
            }
        }
        
        // ItFollowsMod.LOGGER.error("[Stalker] All spawn attempts failed - entity will remain at current position");
    }
    ...
    private BlockPos getCaveSpawnPos(ServerLevel world, BlockPos pos) {
        ...
        for (int y = startY; y > minY; y--) {
            ...
            if (world.canSeeSky(checkPos)) continue;
            
            // Need a solid block below
            BlockPos below = checkPos.below();
            if (!world.getBlockState(below).getMaterial().isSolid()) continue;
            
            // Check that the space is free
            if (world.getBlockState(checkPos).getMaterial().isSolid()) continue;
            if (world.getBlockState(checkPos.above()).getMaterial().isSolid()) continue;
            
            // Check for liquids
            if (world.getBlockState(checkPos).getMaterial().isLiquid()) continue;
            if (world.getBlockState(below).getMaterial().isLiquid()) continue;
    ...
    private BlockPos trySpawnAdjacentToPlayer(ServerLevel world, Player player) {
        ...
        for (int x = -2; x <= 2; x++) {
            for (int z = -2; z <= 2; z++) {
                if (x == 0 && z == 0) continue; // Skip player's position
    ...
    private void checkPlayerDistanceAndRespawn() {
        ...
        long currentTime = this.level.getGameTime();
        if (currentTime - lastDistanceCheckTime < 100) return; // Only check every 5 seconds
        ...
        int respawnDistance = ModConfig.MINIMUM_SPAWN_DISTANCE.get() + this.random.nextInt(ModConfig.MAXIMUM_SPAWN_DISTANCE.get() - ModConfig.MINIMUM_SPAWN_DISTANCE.get() + 1);
        
        if (distanceSquared > respawnDistance * respawnDistance) {
            // ItFollowsMod.LOGGER.info("[Stalker] Player is {} blocks away, respawning closer", Math.sqrt(distanceSquared));
            this.respawnNearby();
    ...
    private boolean isStuck() {
        ...
        if (stuck && !lastReportedStuck) {
            // ItFollowsMod.LOGGER.warn("[Stalker] isStuck: Stalker is stuck at {}", this.blockPosition());
